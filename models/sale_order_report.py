...
class SaleOrder(models.Model):
    ...
    def _compute_amount_tax_qty_delivered(self):
        for order in self:
            order.amount_tax_qty_delivered = order.price_total_qty_delivered - \
                order.price_subtotal_qty_delivered
...
class ExcelSaleOrder(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        date_format = workbook.add_format(
            {'num_format': 'dd/mm/yyyy', 'font_size': 16, 'align': 'center', 'valign': 'vcenter'})
        currency_format = workbook.add_format(
            {'num_format': '[$$-80A]#,##0.00', 'font_size': 16, 'align': 'center', 'valign': 'vcenter'})
        ...
        for order in partners:
            ...
            sheet.write(
                3, 1, order.payment_term_id.name if order.payment_term_id else "N/A", text)
            ...
            for order_line in order.order_line:
                sheet.write(
                    row, 3, f"{order_line.product_id.display_name}", text)
                sheet.write(row, 4, f"{order_line.name}", text)
                sheet.write(
                    row, 5, order_line.qty_delivered if order_line.qty_delivered else 0, text)
                sheet.write(row, 6, order_line.price_unit, currency_format)
                sheet.write(row, 7, ", ".join(
                    [tax.name for tax in order_line.tax_id]), text)
                sheet.write(row, 8, (order_line.qty_delivered if order_line.qty_delivered else 0)
                            * (order_line.price_unit), currency_format)
            ...
            sheet.write(6, 1, order.price_subtotal_qty_delivered,
                        currency_format)
